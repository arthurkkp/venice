...
/**
 * Integration test for ElasticsearchDaVinciRecordTransformer.
 *
 * This test uses Testcontainers to spin up an Elasticsearch container for testing.
 */
public class ElasticsearchDaVinciRecordTransformerIntegrationTest {
  private static final Logger LOGGER = LogManager.getLogger(ElasticsearchDaVinciRecordTransformerIntegrationTest.class);
  ...
  public void setUp() {
    ...
    restClient = RestClient.builder(new HttpHost(elasticsearchHost, elasticsearchPort, "http")).build();
    esClient = new ElasticsearchClient(new RestClientTransport(restClient, new JacksonJsonpMapper()));

    // Start Venice cluster
  ...
  public void testRecordTransformer() throws Exception {
    ...
    try (CachingDaVinciClientFactory factory = getCachingDaVinciClientFactory(
        ...
        backendConfig,
        cluster)) {
      Set<String> fieldsToIndex = Collections.emptySet();
      ...
      esClient.indices().refresh(RefreshRequest.of(r -> r.index(storeName)));

      assertDocumentCount(storeName, DEFAULT_USER_DATA_RECORD_COUNT, "subscribeAll() finishes!");
      ...
      TestUtils.waitForNonDeterministicAssertion(10, TimeUnit.SECONDS, true, () -> {
        esClient.indices().refresh(RefreshRequest.of(r -> r.index(storeName)));
        assertDocumentCount(storeName, DEFAULT_USER_DATA_RECORD_COUNT - 1, "deleting 1 row!");
      });

      clientWithRecordTransformer.unsubscribeAll();
    }

    assertDocumentCount(storeName, DEFAULT_USER_DATA_RECORD_COUNT - 1, "DVC gets closed!");
  }

  private void assertDocumentCount(String indexName, int expectedCount, String assertionErrorMsg) throws IOException {
    SearchResponse<Map> searchResponse = esClient.search(SearchRequest.of(s -> s.index(indexName).size(0)), Map.class);
    long docCount = searchResponse.hits().total().value();
    assertEquals(
        docCount,
        expectedCount,
        "The index should contain " + expectedCount + " documents right after " + assertionErrorMsg);
  }
