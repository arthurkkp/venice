...
/**
 * A Flink sink function for writing data to Venice stores.
 *
 * This class implements Flink's {@link RichSinkFunction} interface and provides
 * functionality equivalent to the Samza {@code VeniceSystemProducer}.
 *
 * Flink jobs talk to either parent or child controller depending on the aggregate mode config.
 * The decision of which controller should be used is made in {@link FlinkVeniceSinkFactory}.
 ...
 */
public class FlinkVeniceSink extends RichSinkFunction<VeniceRecord> implements Closeable {
  private static final long serialVersionUID = 1L;
  private static final Logger LOGGER = LogManager.getLogger(FlinkVeniceSink.class);
  ...
  private final Optional<String> routerUrl;

  // Mutable, lazily initialized state (transient for serialization)
  private transient Schema keySchema;
  private transient String canonicalKeySchemaStr;
  private transient LoadingCache<Schema, String> canonicalSchemaStrCache;
  private transient VeniceConcurrentHashMap<Schema, Pair<Integer, Integer>> valueSchemaToIdsMap;
  private transient VeniceConcurrentHashMap<Pair<Integer, Integer>, Schema> valueSchemaIdsToSchemaMap;

  private transient D2Client primaryControllerColoD2Client;
  private transient D2Client childColoD2Client;
  private transient ControllerClient controllerClient;
  private transient StoreSchemaFetcher schemaFetcher;
  ...
  private transient Map<String, D2ClientEnvelope> d2ZkHostToClientEnvelopeMap;

  /**
   * Constructor for discovery URL mode.
   */
  FlinkVeniceSink(
      String discoveryUrl,
      String storeName,
    ...
    this.routerUrl = Optional.ofNullable(routerUrl);
    ...
    this.additionalConfigs = additionalConfigs != null ? additionalConfigs : new Properties();
  }

  /**
   * Constructor for D2 service discovery mode.
   */
  FlinkVeniceSink(
  ...
  /**
   * Initializes the sink and establishes connections to Venice controllers.
   */
  public synchronized void start() {
    ...
    this.canonicalSchemaStrCache = Caffeine.newBuilder().maximumSize(10).build(AvroCompatibilityHelper::toParsingForm);
    ...
    VersionCreationResponse versionCreationResponse = (VersionCreationResponse) controllerRequestWithRetry(
        () -> this.controllerClient.requestTopicForWrites(
            ...
            pushType,
            jobId,
            true, // sendStartOfPush must be true in order to support batch push to Venice from Flink app
            false, // Flink jobs, including batch ones, are expected to write data out of order
            false,
            partitioners,
            Optional.empty(),
    ...
    if (pushType.isBatchOrStreamReprocessing()) {
      ...
      com.linkedin.venice.meta.Version version = storeResponse.getStore()
  ...
  protected void setupClientsAndReInitProvider() {
    if (discoveryUrl.isPresent()) {
      ...
      if (verifyLatestProtocolPresent && routerUrl.isPresent()) {
        ...
        com.linkedin.venice.schema.SchemaReader kafkaMessageEnvelopeSchemaReader =
  ...
  @Override
  public void invoke(VeniceRecord record, Context context) throws Exception {
    if (!isStarted) {
      throw new VeniceException("FlinkVeniceSink has not been started yet!");
    }

    send(record.getKey(), record.getValue(), record.getLogicalTimestamp());
  }

  protected CompletableFuture<Void> send(Object keyObject, Object valueObject, long logicalTimestamp) {
    ...
    long timestamp = logicalTimestamp;
    // Only transmit the timestamp if this is a realtime topic and timestamp is valid
    if (timestamp <= 0 || !Version.isRealTimeTopic(topicName)) {
      timestamp = VeniceWriter.APP_DEFAULT_LOGICAL_TS;
    }

    if (valueObject == null) {
      veniceWriter.delete(key, timestamp, new CompletableFutureCallback(completableFuture));
    } else {
      ...
      if (valueSchemaIdPair.getSecond() == -1) {
        veniceWriter
            .put(key, value, valueSchemaIdPair.getFirst(), timestamp, new CompletableFutureCallback(completableFuture));
      } else {
  ...
  public synchronized void stop() {
    ...
    if (d2ZkHostToClientEnvelopeMap != null) {
  ...
  AbstractVeniceWriter<byte[], byte[], byte[]> getInternalWriter() {
